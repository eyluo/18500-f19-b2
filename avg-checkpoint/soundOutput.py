...
def listen_for_speech(threshold=THRESHOLD, num_phrases=-1):
    ...
    coeffs = [[0] * len(mfcc_vals[0][0]) for _ in mfcc_vals[0]]
    # usage: mfcc_vals[sample][chunk][coeff]
    for chunk in range(len(mfcc_vals[0])):
        for coeff in range(len(mfcc_vals[0][0])):
            avg = 0
            for sample in range(len(mfcc_vals)):
                avg += mfcc_vals[sample][chunk][coeff]
            avg /= len(mfcc_vals)
            coeffs[chunk][coeff] = avg
